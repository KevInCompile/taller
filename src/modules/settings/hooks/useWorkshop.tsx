...
import { INITIAL_WORKSHOP } from '../states/workshopStates';

export const useWorkshop = () => {
  const [workshopData, setWorkshopData] = useState(INITIAL_WORKSHOP as WorkShop);
  ...
  const fetchWorkshop = async () => {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        setIsCreating(true);
        setWorkshopData(INITIAL_WORKSHOP as WorkShop);
      } else {
