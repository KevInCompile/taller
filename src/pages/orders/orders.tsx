...
import {
  ClipboardList, Search, Plus, Loader2, X, Wrench,
...
import { STATUS_OPTIONS, PRIORITY_OPTIONS } from '../../models/work-order.constants';
import { usePagination } from '../../hooks/usePagination';
import { Pagination } from '../../components/ui/Pagination';
import type { WorkOrder, WorkOrderStatus, WorkOrderPriority } from '../../models/work-order.model';
...
export const OrdersPage = () => {
  ...
  useEffect(() => { resetPage(); }, [searchTerm, statusFilter, priorityFilter, resetPage]);
  ...
  const handleChangeStatus = async (order: WorkOrder) => {
    const inputOptions = Object.fromEntries(
      STATUS_OPTIONS.map(o => [o.value, o.label]),
    );

    const { value: newStatus, isConfirmed } = await Swal.fire<WorkOrderStatus>({
    ...
    if (isConfirmed && newStatus && newStatus !== order.status) {
      try {
